...
if($action == 'msg_tenant'){
	$login = $crud->msg_tenant();
	if($login)
		echo $login;
}

if($action == 'update_tenant'){
	$login = $crud->update_tenant();
	if($login)
		echo $login;
}
...
if($action == 'logout'){
	...
	if($logout)
		echo $logout;
	header("location:home_page.php?page=home");
}
if($action == 'logout2'){
	$logout = $crud->logout2();
	if($logout)
...
if($action == "save_landlord"){
	$save = $crud->save_landlord();
	if($save)
		echo $save;
}
...
if($action == "delete_landlord"){
	$save = $crud->delete_landlord();
	if($save)
		echo $save;
}
