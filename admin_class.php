...
Class Action {
	...
	function update_tenant() {
		...
		$data .= ", email = '$email'";
		echo $data;
		$save = $this->db->query("UPDATE tenants set $data where house_id = $houseid");
		if($save)
			return 1;
	}
	...
	function save_landlord() {
		...
		if(empty($id)){
			...
			$save = $this->db->query("INSERT INTO landlords set $data");
		}else{
			$save = $this->db->query("UPDATE landlords set $data where id = $id");
		}
		if($save)
			return 1;
	}
	...
	function delete_landlord(){
		extract($_POST);
		// $delete = $this->db->query("UPDATE tenants set status = 0 where id = ".$id);
		$delete = $this->db->query("DELETE FROM landlords where id = ".$id);
		if($delete){
			return 1;
		}
	}
	...
	function solved() {
		extract($_POST);
		$delete = $this->db->query("DELETE FROM complaints where sno =".$id);
		if($delete){
			return 1;
		}
	}
